Real-Time Anomaly Detection Pipeline (Serverless & CI/CD)
...
The Flow:
 * Ingestion: Python logic generates synthetic traffic patterns (normal users vs. DDoS/Bot attacks).
 * Processing (AWS Lambda): A serverless function wakes up on a schedule (Cron), ingests the micro-batch, and applies a sliding window aggregation algorithm.
